...
interface FolderStatus {
  folder: string;
  status: "needs_input" | "completed" | "not_started";
  iteration?: number;
  startedAt?: Date;
}

function parseRalphLoopFile(filePath: string): RalphLoopState | null {
  try {
    const content = readFileSync(filePath, "utf-8");
    // Extract YAML frontmatter between --- markers
    const match = content.match(/^---\n([\s\S]*?)\n---/);
    if (!match) return null;
    return parse(match[1]) as RalphLoopState;
  } catch {
    return null;
  }
}
...
async function main() {
  ...
  for (const status of statuses) {
    ...
    switch (status.status) {
      case "needs_input":
        ...
        if (status.startedAt) {
          const state = parseRalphLoopFile(
            resolve(parentDir, status.folder, ".claude/ralph-loop.local.md")
          );
          if (state?.started_at) {
  ...
  console.log(
    `  Summary: ${needsInput} needs input, ${completed} completed, ${notStarted} not started`
  );
  console.log();
}
